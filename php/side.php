<!--
Plugin Name: GL Facebook Likebox
Plugin URI: http://simivar.net/plugins/gl-facebook-likebox/
Description: Adds a great-lookin' Facebook Likebox to Your site.
Author: Krystian 'Simivar' Marcisz
Author URI: http://www.simivar.net/
Version: 1.0.2
Text Domain: glfl
Domain Path: /lang/
-->
	
	<h3><?php _e('About author', 'glfl'); ?></h3>
    <ul class="menu">
   		<li><a href="http://wordpress.org/extend/plugins/profile/simivar"><?php _e('WP Extended profile', 'glfl'); ?></a></li>
    	<li><a href="http://simivar.net/"><?php _e('Homepage', 'glfl'); ?></a></li>
		<li><a href="http://selenagomez.com.pl/"><?php _e('Selena Gomez Polish Fansite', 'glfl'); ?></a></li>
    </ul>
        
    <h3><?php _e('My other plugins', 'glfl'); ?></h3>
    <ul class="menu">
		<li><a href="http://wordpress.org/extend/plugins/advanced-twitter-profile-widget/">Advanced Twitter Profile Widget</a></li>
	</ul>
	
	<h3><?php _e('Thanks a lot to...', 'glfl'); ?></h3>
	<ul class="menu">
		<li><a href="http://facebookicon.net/"><?php _e('FacebookIcon.net for facebook icons', 'glfl'); ?></a></li>
		<li><a href="http://developers.facebook.com/docs/reference/plugins/like-box/"><?php _e('Facebook for LikeBox code', 'glfl'); ?></a></li>
		<li><a href="http://jquery.com/"><?php _e('jQuery team for jQuery 1.4.4', 'glfl'); ?></a></li>
    </ul>
</div>